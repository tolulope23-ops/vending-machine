...
const buttons = document.querySelectorAll("#alpha-num-btn .btn");
const alphaButtons = document.querySelectorAll("#alpha-num-btn .alpha");
...
const vendingStock = {
    machineId: "M001",
    slots: (Array.isArray(retrieveStock === null || retrieveStock === void 0 ? void 0 : retrieveStock.slots) && retrieveStock.slots.length > 0) ? retrieveStock.slots : defaultStockValue
};
...
const displayProducts = () => {
    vendingStock.slots.forEach((slot) => {
        ...
        if (slot.quantity < 1)
            return; //dont display if the product quantity is less than 1.
...
const keyPadInput = () => {
    buttons.forEach((btn) => {
        btn.addEventListener("click", () => {
            var _a;
            const btnValue = (_a = btn.textContent) === null || _a === void 0 ? void 0 : _a.trim();
...
const calculateTotalPrice = (slotcode, quantity) => {
...
const processPayment = () => {
    ...
    if (isCheckoutStage) {
        ...
        ;
        if (userPays < total) {
            updateDisplay(`Insufficient funds. <br> You entered #${userPays}, but total is #${total}.`);
            return;
        }
        ;
        updateDisplay(`
